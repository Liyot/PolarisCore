...
class ModalForm extends BaseForm{
	...
	public function __construct(
		...
		Utils::validateCallableSignature(function(PolarisPlayer $player, bool $choice){ }, $onSubmit);
	...
	public static function confirm(string $title, string $content, \Closure $onConfirm) : self{
		/** @phpstan-ignore-next-line */
		Utils::validateCallableSignature(function(PolarisPlayer $player){ }, $onConfirm);
		return new self($title, $content, static function(PolarisPlayer $player, bool $response) use ($onConfirm) : void{
			if($response){
				$onConfirm($player);
			}
		});
