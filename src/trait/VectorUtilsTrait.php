...
trait VectorUtilsTrait
{
    ...
    /**
     * le nombre de combinaison possible = 62! / ($length! * (62 - $length)!)... franchement c'est pas mal
     *
     * @param int $length
     * @param bool $prefix
     * @return string
     */
    public static function generateSuffix(int $length, bool $prefix = false): string
    {
        $suffix = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
        if ($length > strlen($suffix)) {
            $length = strlen($suffix);
        }
        $random = str_shuffle($suffix);
        if($prefix)
        {
